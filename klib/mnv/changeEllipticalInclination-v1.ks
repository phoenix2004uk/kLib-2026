{
	...
	function changeInclination {
		...
		if obt:inclination >= targetInclination - errorMargin and obt:inclination <= targetInclination + errorMargin {
			return ApiOK(node(time:seconds, 0, 0, 0), "Inclination is within " + errorMargin + "° limit of " + targetInclination + "°: " + obt:inclination + "°").
		}
	...
	export(lex(
		"AN", {
			parameter targetInclination, errorMargin is 0.25.
		...
		"next", {
			...
			if utDN < utAN {
				return changeInclination(targetInclination, utDN, errorMargin, true).
			}
		...
		"highest", {
			...
			local rAN is orbitalParameters:Vr(Van).
			local rDN is orbitalParameters:Vr(Vdn).
			if rDN > rAN {
				return changeInclination(targetInclination, time:seconds + orbitalMechanics:etaV(Vdn), errorMargin, true).
			}
