{
	...
	function createOrbitSnapshot {
		...
		if targetOrbitable:orbit:hasnextpatch {
			set transitionUT to referenceUT + targetOrbitable:orbit:nextpatcheta.
		}

		local positionVector is positionAt(targetOrbitable, referenceUT) - body:position.
		local velocityVector is velocityAt(targetOrbitable, referenceUT):orbit.
		return lex(
			"referenceUT", referenceUT,
			"trueAnomaly", targetOrbitable:orbit:trueanomaly,
			"period", choose targetOrbitable:orbit:period if targetOrbitable:orbit:eccentricity < 1 else false,
			"eccentricity", targetOrbitable:orbit:eccentricity,
			"semimajorAxis", targetOrbitable:orbit:semimajoraxis,
			"apoapsis", choose targetOrbitable:orbit:apoapsis if targetOrbitable:orbit:eccentricity < 1 else false,
			"periapsis", targetOrbitable:orbit:periapsis,
			"planeNormal", (-vcrs(positionVector, velocityVector)):normalized,
	...
	// Get the state on the target orbit at a true-anomaly offset
	function getTargetStateAtOffset {
		parameter targetOrbitable, targetSnapshot, arrivalPhase, arrivalUT, targetPhaseOffset.

		if targetPhaseOffset = 0 return getStateAt(targetOrbitable, arrivalUT).
		return getStateAt(
			targetOrbitable,
			phaseToUT(targetSnapshot, arrivalPhase + targetPhaseOffset)
		).
	}
	...
	function evaluateTransfer {
		parameter targetOrbitable, cell, shipSnapshot, targetSnapshot,
		...
		local targetState is getTargetStateAtOffset(
			targetOrbitable,
			targetSnapshot,
			arrivalPhase,
			arrivalUT,
	...
	function processCell {
		parameter targetOrbitable, cell, evaluationState,
		...
		set evaluationState["evaluated"] to evaluationState:evaluated + 1.
		...
		if cfg:progressEvery > 0 and mod(evaluationState:evaluated, cfg:progressEvery) = 0 {

			print "Level " + evaluationState:level
				+ ": " + evaluationState:evaluated + " evaluated".
		}
	...
	function createCell {
		parameter departurePhaseMin, departurePhaseMax,
			...
			revolutions, direction, branch,
			expandDepartureMin is false,
			expandDepartureMax is false,
			expandArrivalMin is false,
			expandArrivalMax is false.
	...
	function evaluateInitialPhaseGrid {
		parameter targetOrbitable, shipSnapshot, targetSnapshot,
			departureWindow, tofWindow, targetPhaseOffset,
			transferSafetyBounds, cfg.
	...
	function evaluateRefinedPhaseGrid {
		parameter targetOrbitable, parentCandidates, phaseCellSize, level,
			shipSnapshot, targetSnapshot, departureWindow, tofWindow,
	...
	function chaseBoundary {
		parameter targetOrbitable, candidate, phaseCellSize,
			shipSnapshot, targetSnapshot, departureWindow, tofWindow,
			targetPhaseOffset, transferSafetyBounds, cfg.
...

		{
			...
			dPrint("evaluated = " + evaluationState:evaluated).
			dPrint("valid = " + (evaluationState:evaluated - evaluationState:invalid)).
	...
	function adaptiveSearch {
		...
		print "Evaluating initial phase grid at " + cfg:phaseSizes[0] + " degrees".
		...
		do {
			print "Evaluating phase grid level " + level
				+ " at " + cfg:phaseSizes[level] + " degrees".
		...
		if not chaseCandidate:istype("Boolean") {
			print "Chasing final refinement boundary".
	...
	function refreshTransfer {
		parameter targetOrbitable, candidate, targetSnapshot, targetPhaseOffset, transferSafetyBounds.
	...
	function rendezvous {
		parameter targetOrbitable is target, targetPhaseOffset is 0, options is lex().

		if targetOrbitable:typename <> "Vessel" and targetOrbitable:typename <> "Body" {
			return ApiFail("You can only plot a rendezvous to another vessel or body").
		}

		if body <> targetOrbitable:body {
			return ApiFail("Target vessel must be in the same SOI").
		}

		if targetOrbitable:orbit:eccentricity >= 1 {
			return ApiFail("Orbital-phase rendezvous search does not safely support a target hyperbolic trajectory").
		}
		...
		// Freeze one common orbital phase reference.
		local referenceUT is time:seconds.
		local shipSnapshot is createOrbitSnapshot(ship, referenceUT).
		local targetSnapshot is createOrbitSnapshot(targetOrbitable, referenceUT).
...

		{
			...
			dPrint("target = " + targetOrbitable:name).
			dPrint("target phase offset = " + targetPhaseOffset).
