{
	...
	export({
		parameter mnv,tuning_parameters,fitness_function,seek_magnitude_start is 2,seek_magnitude_end is -2.
		...
		for seek_exponent in range(seek_magnitude_start,seek_magnitude_end-1){
			...
			until best_index=0{
				set best_index to 0.
				local base_data is list(time:seconds+mnv:eta,mnv:radialout,mnv:normal,mnv:prograde).
