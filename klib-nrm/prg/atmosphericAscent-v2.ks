{
	...
	local ascentEtaTarget is {
		return max(15,min(75,15+altitude/1e3)).
	}.
	local ascentSteeringDirection is {
		parameter launchDirection.
		return heading(launchDirection,90-vang(up:vector,(choose srfPrograde if altitude<35e3 else prograde):vector)-min(10,max(0,(verticalSpeed-350)*0.1))).
	}.
	local nextAscentLogAltitude is {
		return(floor(altitude/100)+1)*100.
	}.
	local reportAscent is {
	...
	local updateAscentTelemetry is {
		parameter logState,apTarget,wantedThrottle,showDisplay is false.
		local currentQ is ship:q*ATMtokPa.
	...
	local logAscentEvent is {
		parameter logState,phase,apTarget,wantedThrottle.
		reportAscent(phase,apTarget,wantedThrottle,ship:q*ATMtokPa,logState:maxQ,false,true).
	}.
	local insertionComplete is {
	...
	export(lex(
		"executeAscent",{
			...
			until apoapsis>=apTarget{
				...
				local minimumAscentThrottle is 0.
				local availableVerticalTwr is twr:vAvailable().
				if altitude<35e3 or groundSpeed<2*verticalSpeed {
					set minimumAscentThrottle to choose 1 if availableVerticalTwr<=0 else min(1,1.05/availableVerticalTwr).
				}
		...
		"orbitalInsertion",{
			...
			if not insertionComplete(apTarget,apMaxError,etaMin,peTarget) {
				...
				lock steering to velocity:orbit:normalized*cos(pitchOffset)-vxcl(velocity:orbit:normalized,body:position:normalized):normalized*sin(pitchOffset).
