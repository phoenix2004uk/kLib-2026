...
local circularize is import("mnv/circularizeAtApsis-v1"):Ap.
...
local moonAscentApoapsis is 10e3.
...
if currentState = "KerbinSOI" {
	...
	set currentState to SaveState("ReturnTrim").
}
if currentState = "ReturnTrim" {
	trimKerbinReturn(returnPeriapsis).
...
function returnToKerbin {
	...
	dmsg("Planning return to Kerbin at " + returnPeriapsis + "m", true).
...
function trimKerbinReturn {
	parameter returnPeriapsis.

	until not hasNode { remove nextNode. wait 0. }

	notify("Trimming return trajectory").
	dmsg("Planning return trajectory to " + returnPeriapsis + "m", true).

	local trimResult is changeApsisAtUT:Pe(returnPeriapsis, time:seconds + 60).
	if not trimResult:ok {
		notify("Return trim failed - shutting down").
		dmsg("Return trim planning failed: " + trimResult:msg, true).
		shutdown.
	}
