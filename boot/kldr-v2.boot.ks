...
clearScreen.
{
	...
	function _kBoot {
		...
		if status = "PRELAUNCH" {
			...
			if not _kDebug {
				...
				if _kCopyBest(_kBootName, "1:/boot/" + _kBootName + ".ks", _kBootRoots) {
					reboot.
				}
			}
		}
		...
		if _kDebug {
			if _kConnected and exists(_kMissionSource) {
				copyPath(_kMissionSource, _kMain).
			}
		}
			...
			or (
				not exists(_kMain)
				and _kConnected
				and exists(_kMissionSource)
			) {
			copyPath(_kMissionSource, _kMain).
		}

		if not exists(_kMain) {
	...
	function _kFlushDmsg {
		if not exists(_kDmsgBufferFile) {
			return.
		}
	...
	// Tests every source candidate and copies the smallest to the local volume.
	function _kCopyBest {
	...
	// Returns the physical local .ks path for the library.
	function _kEnsureLib {
		...
		if _kDebug and not exists(_kSource) {
			_kPanic("Error! No library source: " + _kLibName).
		}
